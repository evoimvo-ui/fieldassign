    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-72 border-r border-gray-100 overflow-y-auto p-4 bg-white">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-sm font-semibold text-gray-900">Izvještaji</h1>
          <input
            type="date"
        ...
        {loading ? (
          <div className="text-xs text-gray-400 text-center py-8">Učitavanje...</div>
        ) : tasks.length === 0 ? (
          <div className="text-xs text-gray-400 text-center py-8">Nema zadataka</div>
        ) : (
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {!selected ? (
          <div className="flex items-center justify-center h-full">
            <p className="text-sm text-gray-400">Odaberi zadatak za pregled izvještaja</p>
          </div>
        ) : reportLoading ? (
          <div className="text-sm text-gray-400 text-center py-12">Generisanje izvještaja...</div>
        ) : report ? (
          <div className="max-w-2xl">
            ...
            <div className="flex items-start justify-between mb-6">
              ...
              <button className="btn text-xs" onClick={printReport}>🖨 Štampaj</button>
            </div>

            {/* Info grid */}
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="card p-3">
                <div className="text-xs text-gray-400 mb-1">Radnik</div>
              ...
              <div className="card p-3">
                <div className="text-xs text-gray-400 mb-1">Ukupno aktivnosti</div>
            ...
            {(report.gpsCheckpoints.accepted || report.gpsCheckpoints.arrival || report.gpsCheckpoints.completed) && (
              <div className="card p-4 mb-4">
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">GPS Check-ini</div>
                <div className="space-y-2">
                  {report.gpsCheckpoints.accepted && (
                    <div className="flex items-center gap-2 text-xs">
                      <span className="badge badge-accepted">Prihvaćen</span>
                  ...
                  {report.gpsCheckpoints.arrival && (
                    <div className="flex items-center gap-2 text-xs">
                      <span className="badge badge-inprogress">Dolazak</span>
                      <span className="text-gray-500">
                  ...
                  {report.gpsCheckpoints.completed && (
                    <div className="flex items-center gap-2 text-xs">
                      <span className="badge badge-completed">Završeno</span>
                      <span className="text-gray-500">
            ...
            <div className="card p-4">
              <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-4">Timeline aktivnosti</div>
                  ...
                  {report.activities.map((a, i) => (
                    <div key={i} className="flex gap-3 pb-4 last:pb-0">
                      ...
                      <div className="flex-1 pb-1">
                        ...
                        {a.gps?.lat && <div className="text-xs text-brand-600 mt-0.5 ml-12">📍 GPS: {a.gps.lat?.toFixed(5)}, {a.gps.lng?.toFixed(5)}</div>}
                        {a.evidence?.length > 0 && <div className="text-xs text-brand-600 mt-0.5 ml-12">📎 {a.evidence.length} dokaz(a)</div>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="mt-4 text-xs text-gray-400 text-right">
              Izvještaj generisan: {format(new Date(report.generatedAt), 'dd.MM.yyyy. HH:mm')}
            </div>
          </div>
        ) : (
          <div className="text-sm text-gray-400 text-center py-12">Greška pri učitavanju izvještaja</div>
        )}
      </div>
    </div>
  );
