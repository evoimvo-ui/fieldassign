...
export default function ReportsPage() {
  ...
  const ReportListPanel = () => (
    <div className="h-full w-full md:w-72 border-r border-gray-100 overflow-y-auto px-3 sm:px-4 py-4 bg-white flex-shrink-0">
          ...
          {tasks.map(task => (
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 mt-1">
                <span className="text-xs text-gray-400 break-words min-w-0 flex-1">{task.location}</span>
  ...
  const ReportDetailPanel = () => (
    <div className="flex-1 overflow-y-auto px-3 sm:px-6 py-4 sm:py-6 h-full bg-gray-50">
      {!selected ? (
        <div className="flex items-center justify-center h-full min-h-[40vh]">
          <p className="text-sm text-gray-400 text-center px-4">{t('reports.selectTask')}</p>
        </div>
      ) : reportLoading ? (
        ...
        <div className="max-w-2xl mx-auto w-full">
          ...
          <div className="md:hidden flex items-start gap-2 mb-4">
            <button
              ...
              className="min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg text-gray-600 hover:bg-gray-100 -ml-2"
              aria-label="Back"
            >
              ←
            </button>
            <div className="min-w-0 flex-1">
              <h2 className="text-lg font-semibold text-gray-900 break-words">{report.task.title}</h2>
          ...
          <div className="card p-4 mb-4 print-hidden">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {photoPreviews.map((url, i) => (
                <div key={i} className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-lg overflow-hidden border border-gray-200 flex-shrink-0">
              ...
              <button
                ...
                className="w-16 h-16 sm:w-20 sm:h-20 rounded-lg border-2 border-dashed border-gray-200 flex items-center justify-center text-gray-400 hover:border-brand-400 hover:text-brand-400 transition-colors text-xl flex-shrink-0"
          ...
          <div id="printable-area">
            <div className="print-only mb-6 pb-4 border-b-2 border-gray-800">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-lg font-bold text-gray-900 break-words">FieldAssign</div>
                  {organization?.name && (
                    <div className="text-sm text-gray-600 mt-0.5 break-words">{organization.name}</div>
                  )}
                </div>
                    ...
                    <div key={i} className="timeline-item flex gap-3 pb-4 last:pb-0">
                      ...
                      <div className="flex-1 pb-1 min-w-0">
                        <div className="flex flex-wrap items-baseline gap-2">
                          <span className="text-xs font-medium text-gray-400 w-10 flex-shrink-0">{a.time}</span>
            ...
            {photoPreviews.length > 0 && (
              <div className="card p-4 mt-4 print-hidden">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {photoPreviews.map((url, i) => (
                    <img key={i} src={url} alt="" className="w-full h-20 sm:h-24 object-cover rounded-lg border border-gray-100" />
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="print-only mt-12 grid grid-cols-1 sm:grid-cols-2 gap-8 text-xs text-gray-500">
      ...
      {showSendModal && (
        ...
        >
          <div className="modal-panel max-w-sm">
            ...
            <p className="text-xs text-gray-400 mb-4 break-words">{t('reports.sendHint')}</p>
