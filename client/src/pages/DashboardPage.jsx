  ...
  return (
    <div className="px-3 sm:px-6 py-4 sm:py-6 max-w-4xl mx-auto w-full">
      ...
      <div className="mb-5 sm:mb-6">
        ...
        <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5 capitalize break-words">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 mb-5 sm:mb-6">
        {[
          { label: t('dashboard.totalToday'), value: tasks.length, color: 'text-gray-900 dark:text-gray-100' },
          ...
          { label: t('dashboard.pending'), value: pending, color: 'text-gray-500 dark:text-gray-400' },
        ].map((s) => (
          <div key={s.label} className="card p-3 sm:p-4 min-h-[80px]">
            <div className="text-xs text-gray-400 dark:text-gray-500 mb-1 break-words">{s.label}</div>
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 break-words">{t('dashboard.todayTasks')}</h2>
        <button
      ...
      {loading ? (
        ...
        <div className="card p-6 sm:p-8 text-center">
          <div className="text-gray-400 dark:text-gray-500 text-sm break-words">{t('dashboard.noTasks')}</div>
        </div>
      ) : (
        <div className="space-y-2">
          {tasks.map((task) => (
            <div
              key={task._id}
              onClick={() => navigate('/tasks')}
              className="card p-3 sm:p-4 hover:border-gray-200 dark:border-gray-700 cursor-pointer transition-colors"
            >
      ...
      {user?.role === 'admin' && (
        <div className="mt-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 gap-2">
            <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300">{t('dashboard.workerStats')}</h2>
          ...
          {statsLoading ? (
            ...
            <div className="card p-8 text-center text-sm text-gray-400 dark:text-gray-500">{t('dashboard.noStats')}</div>
          ) : (
            <div className="space-y-2">
              {workerStats.map((w) => {
                  ...
                  <div key={w.workerId} className="card p-4">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{w.workerName}</div>
