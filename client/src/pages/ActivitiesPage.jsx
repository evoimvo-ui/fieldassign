  ...
  return (
    <div className="px-3 sm:px-6 py-4 sm:py-6 max-w-2xl mx-auto w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-5 sm:mb-6 gap-3">
        <h1 className="text-base font-semibold text-gray-900 dark:text-gray-100 break-words">{t('activities.title')}</h1>
        <input
          type="date"
      ...
      {loading ? (
        <div className="text-sm text-gray-400 dark:text-gray-500 text-center py-12">{t('common.loading')}</div>
      ) : activities.length === 0 ? (
        <div className="card p-8 sm:p-10 text-center text-sm text-gray-400 dark:text-gray-500 break-words">
        ...
        <div className="card divide-y divide-gray-50 dark:divide-gray-800">
          {activities.map((a, i) => (
            <div key={a._id || i} className="flex items-start gap-3 p-3 sm:p-4">
              <div className="text-xs text-gray-400 dark:text-gray-500 w-12 flex-shrink-0 mt-0.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                {a.task?.title && <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 break-words">{a.task.title}</div>}
                ...
                {a.user?.name && <div className="text-xs text-gray-400 dark:text-gray-500 mt-1 break-words">{a.user.name}</div>}
              </div>
            </div>
          ))}
        </div>
